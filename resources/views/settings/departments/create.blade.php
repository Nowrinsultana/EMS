<x-layout>
    <div class="max-w-md mx-auto mt-10 bg-white p-6 rounded-lg shadow">
        ...
        <form method="POST" action="{{ route('settings.departments.store') }}">
            ...
            <div class="mb-4">
                ...
                <select id="department_head_id" name="department_head_id"
                    ...
                    @foreach ($employees as $emp)
                        <option value="{{ $emp->id }}" {{ old('department_head_id') == $emp->id ? 'selected' : '' }}>{{ $emp->name }}</option>
                    @endforeach
                </select>
            ...
            <button type="submit" class="w-full py-2 px-4 bg-indigo-600 text-white rounded hover:bg-indigo-700">Create Department</button>
        </form>
    </div>
</x-layout>
