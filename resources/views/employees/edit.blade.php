<x-layout>
    <div class="max-w-md mx-auto mt-10 bg-white p-6 rounded-lg shadow">
        ...
        <form method="POST" action="{{ route('employees.update', ['dptid' => $dptid, 'employee' => $employee]) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input id="date_of_birth" type="date" name="date_of_birth" value="{{ old('date_of_birth', $employee->date_of_birth?->format('Y-m-d')) }}"
                       class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                @error('date_of_birth')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input id="passport_number" type="text" name="passport_number" value="{{ old('passport_number', $employee->passport_number) }}"
                       class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div class="mb-4">
                ...
                <input id="start_date" type="date" name="start_date" value="{{ old('start_date', $employee->start_date?->format('Y-m-d')) }}"
                       class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                @error('start_date')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input id="leave_balance" type="number" name="leave_balance" value="{{ old('leave_balance', $employee->leave_balance) }}" min="0"
                       class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                @error('leave_balance')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
